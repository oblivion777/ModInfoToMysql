...
function toPV(page = "test") {
    pool.execute(`SELECT page,count FROM pv WHERE page= ? `, [page], function (err, result) {
        //console.log(result);
        pool.query(`UPDATE pv SET count=count+1 WHERE page="total_page_views"`);
        if (result[0] == undefined) {
            return pool.execute(`INSERT INTO pv (page) VALUE (?)`, [page]);
        } else {
            return pool.execute(`UPDATE pv SET count=count+1 WHERE page= ? `, [page]);
        };
    });
}
...
//写入UV数据
function toIP(ip) {
    pool.query(`SELECT ip,count FROM ip WHERE ip= ? `, [ip], function (err, result) {
        //console.log(result);
        if (result[0] == undefined) {
            return pool.query(`INSERT INTO ip (ip) VALUE (?)`, [ip]);
        } else {
            return pool.query(`UPDATE ip SET count=count+1 WHERE ip= ? `, [ip]);
        };
    });
}
