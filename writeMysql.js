...
function toPV(page = "test") {
    ...
    connectMysql.query(`SELECT page,count FROM pv WHERE page="${page}"`, function (err, result) {
        //console.log(result);
        connectMysql.query(`UPDATE pv SET count=count+1 WHERE page="total_page_views"`);
        if (result[0] == undefined) {
            return connectMysql.query(`INSERT INTO pv (page) VALUE ("${page}")`);
        } else {
            return connectMysql.query(`UPDATE pv SET count=count+1 WHERE page="${page}"`);
        };
    });
}
